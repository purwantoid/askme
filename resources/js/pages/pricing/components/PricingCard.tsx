...
export const PricingCard = ({ tier, paymentFrequency }: { tier: (typeof TIERS)[0]; paymentFrequency: string }) => {
    ...
    return (
        ...
        >
            ...
            <h2 className="flex items-center gap-3 text-xl font-medium capitalize">
                {tier.name}
                {isPopular && <Badge className="mt-1 bg-orange-900 px-1 py-0 text-white hover:bg-orange-900">🔥 Most Popular</Badge>}
            </h2>
            ...
            <div className="flex-1 space-y-2">
                ...
                <ul className="space-y-2">
                    {tier.features.map((feature, index) => (
                        <li
                            key={index}
                            className={cn('flex items-center gap-2 text-sm font-medium', isHighlighted ? 'text-background' : 'text-foreground/60')}
                        >
            ...
            <Button
                ...
                Icon={ArrowRight}
                iconPlacement="right"
                className={cn('h-fit w-full rounded-lg', isHighlighted && 'bg-accent text-foreground hover:bg-accent/95')}
            >
                {tier.cta}
            </Button>
        </div>
